...
def load_dimentions(data):
    """Take our data set and reformat it removing groundings on less than
    8000 words and remove all words not rated on all remaining groundings."""
    ...
    good_indlist = set(list(range(0,55)))-set(ind_lis)
    ...
    goodnames = []    
    for b in good_indlist:
        goodnames.append(data.columns[b])
    ...
    A_array = np.vstack([goodnames,A_array])
...
def percent_compare_ordered(G1,G1N,G2,G2N,D=True,pathname="N/A"):
...
def combine_raw(raw1,raw2,H):
    ...
    Warning: doesn't check if the raw data has normalised same word ratings"""
...
def area_estimation(x,y,l=1):
    
    x2,y2 = x[:round(len(x)/l)],y[:round(len(x)/l)]
    area = np.trapz(y2,x2)
    
    return area
    
def list_transpositions(strings):
    flipped_list = []
    for i in range(len(strings)):
        if i % 2 == 0 and i < len(strings) - 1:
            flipped_list.append(strings[i + 1])
            flipped_list.append(strings[i])
        elif i % 2 == 0:
            flipped_list.append(strings[i])
    return flipped_list
